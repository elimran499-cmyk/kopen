    ...
    >
      ...
      <div className="flex items-baseline">
        {WORD.split('').map((letter, i) => (
          ...
          >
            {/* The flex row makes every letter its own item, and an item holding
                only a normal space collapses to zero width — so the gap in
                "IPTV Koning" would disappear. A non-breaking space keeps it. */}
            {letter === ' ' ? ' ' : letter}
          </span>
        ))}
        <span
